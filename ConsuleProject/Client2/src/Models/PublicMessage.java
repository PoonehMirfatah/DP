...
public class PublicMessage implements Serializable {
    ...
    public static String showMessages() throws Exception {
        ...
        MySQLConnection sql = new MySQLConnection();
        String SQLcmd = "Select allusers.UserName,publicmessages.messsage,publicmessages.dateAndTime FROM allusers INNER JOIN publicmessages on allusers.UserID=publicmessages.UserID ";
        ResultSet rs = sql.executeQuery(SQLcmd);
        if (rs == null) {
            return "Not Found!";
        } else {
    ...
    public static String showSearchedTime(String time1,String time2) throws SQLException {
        StringBuilder sb = new StringBuilder();
        MySQLConnection sql = new MySQLConnection();
        String SQLcmd = String.format("Select allusers.UserName,publicmessages.messsage,publicmessages.dateAndTime FROM allusers INNER JOIN publicmessages on allusers.UserID=publicmessages.UserID WHERE publicmessages.dateAndTime>'%s' AND publicmessages.dateAndTime<'%s'",time1,time2);
        ResultSet rs = sql.executeQuery(SQLcmd);
        if (rs == null) {
            return "Not Found!";
        } else {
    ...
    public static String showSearchedName(String name) throws SQLException {
        StringBuilder sb = new StringBuilder();
        MySQLConnection sql = new MySQLConnection();
        String SQLcmd = String.format("Select allusers.UserName,publicmessages.messsage,publicmessages.dateAndTime FROM allusers INNER JOIN publicmessages on allusers.UserID=publicmessages.UserID WHERE allusers.UserName='%s'",name);
        ResultSet rs = sql.executeQuery(SQLcmd);
        if (rs == null) {
            return "Not Found!";
        } else {
