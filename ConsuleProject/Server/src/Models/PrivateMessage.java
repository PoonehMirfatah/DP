...
public class PrivateMessage implements Serializable {
    ...
    public static String showPvMessages(String sender,String reciever) throws Exception {
        ...
        MySQLConnection sql = new MySQLConnection();
        String SQLcmd = String.format("Select pvmessages.Sender,pvmessages.Reciever,pvmessages.Content, pvmessages.dateAndTime FROM pvmessages WHERE pvmessages.Sender='%s' AND pvmessages.Reciever='%s' OR pvmessages.Sender='%s' AND pvmessages.Reciever='%s' ORDER BY ID",sender,reciever,reciever,sender);
        ResultSet rs = sql.executeQuery(SQLcmd);
        if (rs == null) {
            return "Not Found!";
        } else {
